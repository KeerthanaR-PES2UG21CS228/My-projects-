...
FOR EACH ROW BEGIN
    INSERT INTO `order_log` (`order_ID`, `username`, `foodname`, `price`, `quantity`, `order_date`)
    VALUES (NEW.order_ID, NEW.username, NEW.foodname, NEW.price, NEW.quantity, NEW.order_date);
END;
...
CREATE PROCEDURE `get_orders_by_customer`(`username` VARCHAR(30))
BEGIN
    SELECT * FROM `orders`
    WHERE `username` = username;
END;

CREATE PROCEDURE `get_orders_by_restaurant`(`R_ID` INT)
BEGIN
    SELECT * FROM `orders`
    WHERE `R_ID` = R_ID;
END;
